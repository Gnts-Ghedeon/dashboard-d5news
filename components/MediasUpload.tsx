...
const MediasUpload = () => {
    ...
    return (
        <div className="overflow-x-auto rounded-lg border border-stroke bg-white shadow-default  dark:border-strokedark dark:bg-boxdark">
            ...
            <div className="border-stroke py-4 px-6.5 dark:border-strokedark border-t flex flex-col gap-4">
            {
                selectedMedias.map((selectedMedia: Media, i: number) => {
                    return (
                        <div key={i} className="">
                            <RenderFileThumbnail file={selectedMedia} />
                        </div>
                    )
                })
            }
...
const RenderFileThumbnail = ({ file }: { file: Media }) => {
    switch (file.type) {
        case "image":
            return (
                <Link target="_blank" href={file.url} className="flex gap-2">
        ...
        case "video":
            return (
                <Link target="_blank" href={file.url} className="flex gap-2">
                    <div className="h-12 w-12 relative overflow-hidden rounded-md">
                        <video src={file.url} className="h-12 w-12"></video>
                    </div>
                    <div className="flex-1">
                        <span>{file.name}</span>
                    </div>
