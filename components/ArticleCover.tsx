...
const ArticleCover = ({ file, accept, name }: ArticleCoverProps) => {
    ...
    const handleFileUpload = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (files && files.length > 0) {
            ...
            setType(getFileType(selected.name))
        }
    }
    ...
    return (
        <>
            ...
            />
            {
                previewURL
                    ? (
                        <div className="relative h-52 group w-full">
                            {
                                type === "IMAGE"
                                    ? (
                                        <Image
                            ...
                            <div className="hidden group-hover:block absolute w-full bottom-0 bg-black">
                                <button
                                    type="button"
                                    className="px-4 py-2 text-center w-full text-sm text-danger" onClick={() => setPreviewURL(null)}
                                >
                                    Supprimer
                                </button>
                            </div>
                    ...
                    : (
                        ...
                        >
                            <div onClick={handleClick} className="flex flex-col items-center justify-center space-y-3 h-36">
                                ...
                                <p className="text-center">
                                    <span className="text-primary text-sm">Cliquez pour selectionner un fichier</span>
                                </p>
                            </div>
                        </div>
                    )
